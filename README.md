...
If you are running the room NPR10x10x10-S5,5,5-Rx,2.5,2.5, there are some other inputs to be changed such as the deltax, dx = 0.1 and the deltat, dt = 1/100000.
...
9. If you are running the room NPR10x10x10-S5,5,5-Rx,2.5,2.5, go directly to step 12.
10. If you have done all the steps above, open the third python script `3-OptimizationDx.py` and press run. The results will be saved in the local folder `results_diff_opt`.
The time length of the optimization simulation is difficult to quantify as it depends on how much different are the SPL curve over the distance between the radiosity method and the diffusion equation method. It will run until 100% as many times as the quantity C2, C1, and C0 will need to be optimised to minimise the difference between the two curves. The calculation could be in between 5 to 15 minutes, depending on the room that you are simulating and on the optimization variables.
11. Open the fourth python script `4-DiffEq3DWithNewDx.py` and press run. The results will be saved in the local folder `results_diff_opt`.
12. Once you have all the results, you can create figures and reproduce the same figures as per Conference paper.
13. Open the matlab file `A_ComparisonRadiosityDiffusionAfterOpt.m` and run. It should give the same figures as per the conference paper for the specific room you have simulated. If you are running the room NPR10x10x10-S5,5,5-Rx,2.5,2.5, you have completed the workflow and obtained all the figures for the room NPR10x10x10-S5,5,5-Rx,2.5,2.5 of the conference paper. 
14. Open the matlab file `B_ComparisonDx` and run. This will give you the results of comparison between diffusion coefficients of different rooms.
